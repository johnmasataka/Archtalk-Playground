...
17. IMPORTANT: The parameter "verticalPosition" and "position" of the window and door should be lower than 1500mm. 
...
export const validateBuildingData = (data) => {
  ...
  for (let i = 0; i < building.floors.length; i++) {
    ...
    // Check if this is a roof or special structure floor
    const floorNameLower = floor.name.toLowerCase();
    const isRoofStructure = floorNameLower.includes('roof') || 
                           floorNameLower.includes('attic') || 
                           floorNameLower.includes('loft');
    const isHighFloor = i >= building.floors.length - 2;
    
    // Special handling for roof structures
    if (isRoofStructure) {
      console.log(`Detected roof structure at floor ${i + 1}, applying flexible validation`);
      
      // Check if floor has a roof property, which is more important for roof structures
      if (floor.roof && typeof floor.roof === 'object') {
        // If it's a valid roof structure, continue to next floor
        continue;
      }
      
      // If no roof property but it has valid rooms, we'll still validate those
      if (!floor.rooms) {
        // Create an empty rooms array for roof structures without rooms
        floor.rooms = [];
        console.log(`Added empty rooms array to roof structure at floor ${i + 1}`);
      }
    }
    
    // For non-roof floors, enforce rooms must be an array
    if (!Array.isArray(floor.rooms)) {
      console.error(`Invalid rooms property for floor ${i + 1} (${floor.name})`);
      return { valid: false, error: `Invalid room data for floor ${i + 1}` };
    }
    
    // For non-roof floors, rooms array shouldn't be empty
    if (floor.rooms.length === 0 && !isRoofStructure && !isHighFloor) {
      console.error(`Empty rooms array for floor ${i + 1} (${floor.name})`);
      return { valid: false, error: `Invalid room data for floor ${i + 1}` };
    }

    // Check each room (only if there are rooms to check)
    for (let j = 0; j < floor.rooms.length; j++) {
      ...
      // Get room name in lowercase for easier comparison
      const roomNameLower = room.name.toLowerCase();
      
      // List of room types that can have empty walls
      const openSpaceTypes = ['balcony', 'terrace', 'engawa', 'patio', 'deck', 'porch', 'veranda', 'pergola', 'pavilion', 
                              'garden', 'courtyard', 'atrium', 'lanai', 'gazebo', 'outdoor', 'open'];
      
      // Check if this room type should be allowed to have empty walls
      const isOpenSpaceRoom = openSpaceTypes.some(type => roomNameLower.includes(type));
      
      // Check walls property - allow empty walls array for specific room types
      ...
      // Only enforce non-empty walls for regular rooms
      if (!isOpenSpaceRoom && room.walls.length === 0) {
        console.error(`Invalid walls for room ${j + 1} (${room.name}) on floor ${i + 1}`);
        return { valid: false, error: `Invalid walls for room ${j + 1} on floor ${i + 1}` };
      }
      
      // Check each wall (if there are any)
